...
const Settings = () => {
  ...
  const handleProfile = useCallback(async () => {
    const resp = await handleFetch('GET', '/profile');
    if (resp && resp.user) {
      setData(resp.user);
    }
  }, []);
  ...
  const handleReset = async (email) => {
    ...
    if (password) {
      const isvalid_pass = validatePassword(password);
      if (isvalid_pass.isValid) {
        await handleFetch("POST", "/resetpassword", { email, password });
      } else {
        fireToast(isvalid_pass.message, isvalid_pass.isValid);
      }
    }
  };
